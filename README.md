...
**통과/차단 이진 판정이 아니라 점수 기반이다.** 룰마다 점수를 매겨 합산하고 구간별로 조치를 나눈다.

| 점수 | 판정 | 조치 |
|---|---|---|
| 0~39 | `APPROVE` | 승인 |
| 40~69 | `REVIEW` | 승인하되 표시 (실무라면 추가 인증·사후 모니터링) |
| 70~ | `BLOCK` | 차단 (`94`) |

정상 고객의 결제를 막는 비용이 사기 손실보다 클 수 있어 **차단은 확실할 때만** 한다.
단일 룰로는 대부분 차단 구간에 도달하지 않고, 신호가 겹칠 때 올라간다.

### 룰 카탈로그

| 룰 | 조건 | 점수 | 근거 |
|---|---|---|---|
| `VELOCITY_COUNT` | 1분 내 4건 이상 | 60 | 도난 카드 연속 사용 |
| `CARD_TESTING` | 1분 내 소액(1만 미만) 3건 이상 | 50 | 유출 카드 유효성 확인 공격 |
| `VELOCITY_AMOUNT` | 10분 내 누적 300만 초과 | 50 | 한도 소진 시도 |
| `MERCHANT_DIVERSITY` | 5분 내 서로 다른 가맹점 3곳 이상 | 40 | 도난 카드는 가맹점을 옮겨 다닌다 |
| `AMOUNT_ANOMALY` | 평소 평균의 5배 초과 | 30 | 이상 고액 |
| `MIDNIGHT_HIGH_AMOUNT` | 00~06시 100만 초과 | 25 | 시간대 리스크 |
| `CARD_STATUS` | 미등록 / 정지 카드 | — | **즉시 차단** (`14`) |

`CARD_STATUS`만 점수 대상이 아니다. 확정된 사실이므로 임계치 조정에 영향받으면 안 된다.
나머지는 확률적 신호라 점수로 다룬다.

임계치는 `application.yml`의 `fds.rule.*`로 뺐다 — 실무에서 임계치는 오탐률을 보며 계속 조정되므로 배포 없이 바꿀 수 있어야 한다.

### 두 종류의 과거 데이터

| | 단기 이력 (velocity 계열) | 장기 프로파일 (`AMOUNT_ANOMALY`) |
|---|---|---|
| 보는 범위 | 최근 몇 분 | 최근 수개월 |
| 저장 | 인메모리 (실무는 Redis) | DB `card_profiles` |
| 갱신 | 거래마다 실시간 | 야간 배치가 집계 |
| 묻는 것 | "지금 몰아치는가" | "이 사람답지 않은가" |

실시간 판정 경로에서 수개월치를 매번 집계할 수 없으므로 프로파일은 미리 계산된 것을 조회만 한다.
**이 프로젝트는 집계 배치를 만들지 않고 시드 데이터로 대체했다.** 프로파일 표본이 부족하거나(신규 카드) 없으면 `AMOUNT_ANOMALY`는 판정을 보류한다 — 근거 없는 차단을 만들지 않기 위해서다.

### 멱등키와의 관계

FDS의 velocity와 승인 서비스의 멱등성은 **목적이 다르다.**
- velocity: "서로 다른 결제"가 몰아치는가 → 차단
- 멱등성: "같은 결제"가 다시 왔는가 → 저장된 결과 반환

그래서 같은 멱등키의 재시도는 velocity 이력에 중복 집계하지 않는다.
그러지 않으면 시스템 장애 후의 정당한 재시도가 이상거래로 차단된다.

차단도 **HTTP 200 + 응답코드**로 응답한다. 비2xx로 주면 호출하는 쪽의 Feign이 예외로 처리해 "카드사 장애"로 오인한다.
응답에 점수와 적발 룰을 함께 실어 보내 판정 근거를 남긴다 — 근거를 설명하지 못하면 오탐이 나도 어디를 고칠지 알 수 없다.

| Method | URI | 설명 |
|---|---|---|
| `POST` | `/api/fds/inspect` | 판정 (결과 + 점수 + 적발 룰 + 실제 카드 종류) |
